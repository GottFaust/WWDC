...
public class Maximizer{
  ...
  public void Maximizer(){	  
    ...
    try {
    if(!maximizerResults.exists()) maximizerResults.createNewFile();    	   	
    BufferedWriter writer = new BufferedWriter(new FileWriter(maximizerResults));                   
	  
	  for(int a = q; a < modCount; a++) {
		  if(touchModOne == true) Main.selectedWeapon.setModOne(a);
		  		  
		  		for(int s = w; s < modCount; s++) {
		  			if(touchModTwo == true) Main.selectedWeapon.setModTwo(s);  
			  		  
			  		for(int d = e; d < modCount; d++) {
			  			if(touchModThree == true) Main.selectedWeapon.setModThree(d);  
				  		  
				  		for(int f = r; f < modCount; f++) {
				  			if(touchModFour == true) Main.selectedWeapon.setModFour(f);  
					  		  
					  		for(int g = t; g < modCount; g++) {
					  			if(touchModFive == true) Main.selectedWeapon.setModFive(g);  
						  		  
						  		for(int h = y; h < modCount; h++) {
						  			if(touchModSix == true) Main.selectedWeapon.setModSix(h);  
							  		  
							  		for(int j = u; j < modCount; j++) {
							  			if(touchModSeven == true) Main.selectedWeapon.setModSeven(j);  
								  		  
								  		for(int k = i; k < modCount; k++) {
								  			if(touchModEight == true) Main.selectedWeapon.setModEight(k);  
    ...
    //for each target: find its fastest kill time, DPS, etc
        ...
        double average = 0;
        double minmax = 0;
        Vector<TTKresult> filteredResults = new Vector<TTKresult>();
    	...
    	for(int k = 0; k < targets; k++) {
    		filteredResults = new Vector<TTKresult>();
    		filteredResults.addAll(results);
      	  for(int p = results.size()-1; p > 0; p--){
    		  if(Double.parseDouble(filteredResults.get(p).TTKs.get(1+2*k)) < Double.parseDouble(filteredResults.get(p-1).TTKs.get(1+2*k))) {
    			  filteredResults.remove(p-1);
    		  }else {
    			  filteredResults.remove(p);
    		  }
    	  }
    	  Main.output.append("::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::" + "\n");
    	  Main.output.append("Build with the fasted kill time on " + filteredResults.get(0).TTKs.get(2*k) + "\n");
    	  Main.output.append(filteredResults.get(0).build1+"\n");
    	  Main.output.append(filteredResults.get(0).build2+"\n");
    	  Main.output.append("DPS: " + filteredResults.get(0).DPS+"\n");
    	  Main.output.append("Time: " + filteredResults.get(0).TTKs.get(1+2*k)+" seconds" + "\n");
        }	
		...
		filteredResults.addAll(results);
    	  for(int p = results.size()-1; p > 0; p--){
  		  if(filteredResults.get(p).DPS > filteredResults.get(p-1).DPS) {
  			  filteredResults.remove(p-1);
  		  }else {
  			  filteredResults.remove(p);
  		  }
  		...
  		filteredResults.addAll(results);
      	  for(int p = results.size()-1; p > 0; p--){
    		  if(filteredResults.get(p).average < filteredResults.get(p-1).average) {
    			  filteredResults.remove(p-1);
    		  }else {
    			  filteredResults.remove(p);
    		  }
    	  ...
    	  Main.output.append("::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::" + "\n");
      	  ...
      	  Main.output.append(filteredResults.get(0).build1+"\n");
      	  Main.output.append(filteredResults.get(0).build2+"\n");
      	  Main.output.append("Average time to kill: " + filteredResults.get(0).average+"\n");
    	  Main.output.append("--------------------------Individual TTKs----------------------------" + "\n");
      	  for(int k = 0; k < targets; k++) {
          	  Main.output.append(filteredResults.get(0).TTKs.get(2*k) + ": " + filteredResults.get(0).TTKs.get(1+2*k)+" seconds" + "\n");  
      	  }
    		...
    		filteredResults.addAll(results);
        	  for(int p = results.size()-1; p > 0; p--){
      		  if(filteredResults.get(p).minmax < filteredResults.get(p-1).minmax) {
      			  filteredResults.remove(p-1);
      		  }else {
      			  filteredResults.remove(p);
      		  }
        	  ...
        	  Main.output.append(filteredResults.get(0).build1+"\n");
        	  Main.output.append(filteredResults.get(0).build2+"\n");
        	  Main.output.append("Maximum kill time: " + filteredResults.get(0).minmax+"\n");
        	  Main.output.append("--------------------------Individual TTKs----------------------------" + "\n");
          	  for(int k = 0; k < targets; k++) {
              	  Main.output.append(filteredResults.get(0).TTKs.get(2*k) + ": " + filteredResults.get(0).TTKs.get(1+2*k)+" seconds" + "\n");  
          	  }
    ...
    //Clean out the mods
	  if(touchModOne == true) Main.selectedWeapon.setModOne(0);
	  if(touchModTwo == true) Main.selectedWeapon.setModTwo(0);
	  if(touchModThree == true) Main.selectedWeapon.setModThree(0);
	  if(touchModFour == true) Main.selectedWeapon.setModFour(0);
	  if(touchModFive == true) Main.selectedWeapon.setModFive(0);
	  if(touchModSix == true) Main.selectedWeapon.setModSix(0);
	  if(touchModSeven == true) Main.selectedWeapon.setModSeven(0);
	  if(touchModEight == true) Main.selectedWeapon.setModEight(0);
  }
